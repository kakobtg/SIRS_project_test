#!/usr/bin/env bash
# Configure static IP for VM1 (DMZ). Defaults match README (10.0.0.10/24).
set -euo pipefail

IP_CIDR="${IP_CIDR:-10.0.0.10/24}"
GATEWAY="${GATEWAY:-}"   # optional gateway (leave empty for internal-only)
DNS="${DNS:-}"           # optional comma-separated DNS servers
CONNECTION="${CONNECTION:-}"  # NetworkManager connection name; auto-detected if empty
...
echo "[vm1] Using connection: ${CONNECTION}"
echo "[vm1] Setting static IP: ${IP_CIDR}"
nmcli con mod "${CONNECTION}" ipv4.addresses "${IP_CIDR}" ipv4.method manual

if [[ -n "${GATEWAY}" ]]; then
  echo "[vm1] Setting gateway: ${GATEWAY}"
