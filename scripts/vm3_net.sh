#!/usr/bin/env bash
# Configure static IP for VM3 (client). Defaults match README (10.0.2.10/24).
set -euo pipefail

IP_CIDR="${IP_CIDR:-10.0.2.10/24}"
GATEWAY="${GATEWAY:-}"
DNS="${DNS:-}"
...
echo "[vm3] Using connection: ${CONNECTION}"
echo "[vm3] Setting static IP: ${IP_CIDR}"
nmcli con mod "${CONNECTION}" ipv4.addresses "${IP_CIDR}" ipv4.method manual

if [[ -n "${GATEWAY}" ]]; then
  echo "[vm3] Setting gateway: ${GATEWAY}"
